...
TARGET_DATES = [datetime.date(2026, 4, 23), datetime.date(2026, 4, 24)]
OPEN_HOUR = 9
CLOSE_HOUR = 20  # exclusive
...
def main():
    ...
    try:
        ...
        item_rows = []
        delivery_rows = []
        seq = random.randint(50000, 99000)

        for day in TARGET_DATES:
            if day.weekday() == 6:  # Sunday off
                continue
            n = random.randint(28, 62)
            for _ in range(n):
                ...
                order_no = "ORD{}U{:05d}".format(created_at.strftime("%Y%m%d%H%M"), seq)
                seq += 1
        ...
        print(f"Generated {len(order_rows)} orders for 2026-04-23 and 2026-04-24.")
    finally:
        conn.close()
