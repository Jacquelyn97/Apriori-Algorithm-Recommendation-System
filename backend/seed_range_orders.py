...
START_DATE = datetime.date(2026, 3, 27)
END_DATE = datetime.date(2026, 4, 2)  # 含明天（预答辩）
OPEN_HOUR = 9
CLOSE_HOUR = 20  # exclusive, last minute 19:59
...
def main():
    ...
    try:
        ...
        item_rows = []
        delivery_rows = []
        seq = random.randint(30000, 99000)
        ...
        for i in range(total_days):
            ...
            for _ in range(n):
                ...
                created_at = datetime.datetime.combine(day, datetime.time(OPEN_HOUR, 0)) + datetime.timedelta(minutes=minutes)
                paid_at = None
                finished_at = None
                ...
                order_no = "ORD{}R{:05d}".format(created_at.strftime("%Y%m%d%H%M"), seq)
                seq += 1
                ...
                if len(chosen) < num_items:
                    rem = [p for p in products if p not in chosen]
                    if rem:
                        k = min(len(rem), num_items - len(chosen))
                        chosen.extend(random.sample(rem, k=k))
        ...
        with conn.cursor() as cur:
            cur.execute(f"SELECT id, order_no FROM orders WHERE order_no IN ({placeholders})", order_nos)
